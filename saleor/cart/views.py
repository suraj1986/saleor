    ...
    if form.is_valid():
        ...
        if updated_line:
            response['subtotal'] = currencyfmt(
                updated_line.get_total(discounts=discounts).gross.amount,
        ...
        if cart:
            ...
            if local_cart_total is not None:
                response['localTotal'] = currencyfmt(
                    local_cart_total.gross.amount, local_cart_total.currency)
        status = 200
...
def summary(request, cart):
    ...
    def prepare_line_data(line):
        ...
        return {
            ...
            'price_per_item': currencyfmt(
                price_per_item.gross.amount, price_per_item.currency),
            'line_total': currencyfmt(
                line_total.gross.amount, line_total.currency),
            'update_url': reverse(
    ...
    if cart.quantity == 0:
        ...
        data = {
            ...
            'total': currencyfmt(cart_total.gross.amount, cart_total.currency),
